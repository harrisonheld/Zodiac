...
public class EntitySerializer
{
    ...
    public static GameObject EntityFromBlueprint(string blueprintId, Vector2Int pos)
    {
    ...
    {
        XDocument doc = XDocument.Load(BLUEPRINTS_PATH);

        // use an internal method so we dont reload the doc recursively
        GameObject result = EntityFromBlueprintInternal(blueprintId, doc);
        return result;
    }
    private static GameObject EntityFromBlueprintInternal(string blueprintId, XDocument doc)
    {
        // find the element
        XElement blueprintElement = doc.Root.Elements(ENTITY)
               .Where(e => e.Attribute(ID).Value == blueprintId)
               .First();

        // check if it inherits from any other blueprint
        GameObject baseEntity = null;
        XAttribute inheritsAttr = blueprintElement.Attribute(INHERITS);
        if (inheritsAttr == null)
            baseEntity = new GameObject();
        else
            baseEntity = EntityFromBlueprintInternal(inheritsAttr.Value, doc);

       // add components
       foreach(XElement compElement in blueprintElement.Elements(COMPONENT))
       {
            Type componentType = Type.GetType(compElement.Attribute(COMPONENT_TYPE).Value);
            ...
            Component preexistingComponent = baseEntity.GetComponent(componentType);
            ...
            else
            {
                ...
                zodiacComponent = baseEntity.AddComponent(componentType) as ZodiacComponent;
            }

            foreach(XElement propElement in compElement.Elements(PROPERTY))
            {
                string propName = propElement.Attribute(PROPERTY_NAME).Value;
                string propValString = propElement.Attribute(PROPERTY_VALUE).Value;
